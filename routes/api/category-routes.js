...
router.get('/:id', (req, res) => {
  ...
  Category.findOne({
    ...
    .then((results) => {
      // if there's no match, set status to 404 and tell user there is no match with that ID
      if (!results) {
        res.status(404).json({
...
router.put('/:id', (req, res) => {
  ...
  Category.update(
    ...
    .then((results) => {
      // if there are no results, set status to 404 and tell user that ID is not found
      if (!results) {
        res.status(404).json({
